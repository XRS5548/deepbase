...
import { db } from "@/db"
import { databases, forms, dateTriggers, teamMembers, workflows } from "@/db/schema"
...
export async function GET() {
  try {
    ...
    const [d, f, t, tm, w] = await Promise.all([
      ...
      db.select({ v: sql<string>`count(*)` }).from(workflows).where(eq(workflows.createdBy, userId)),
    ])
